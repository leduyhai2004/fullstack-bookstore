...
interface IProps {
    ...
    setDataUpdateBook: (data: IBookTable | null) => void,
    refreshTable: () => void;
}
interface FieldType {
    _id: string;
...
const UpdateBook = (props: IProps) => {
    ...
    useEffect(() => {
        if(dataUpdateBook){
           ...
           const arrSliders = dataUpdateBook.slider.map(item => ({
                uid : uuidv4(),
                name : item,
                status : 'done',
            ...
            setFileListSliders(arrSliders as any);
        }
    }, [dataUpdateBook]);
    ...
    return (
        <Modal
            ...
            okText={isSubmit ? 'Updating...' : 'Update'}
            width={800}
            style={{ top: 20 }}
        >
            <Divider />
